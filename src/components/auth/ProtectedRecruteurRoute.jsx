...
const ProtectedRecruteurRoute = ({ children }) => {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      // Si l'utilisateur n'est pas un recruteur, rediriger vers la page d'accueil
      if (user.user_type?.toLowerCase() !== 'recruteur') {
  ...
  if (!isAuthenticated || !user || user.user_type?.toLowerCase() !== 'recruteur') {
    return null;
  }
...
export default ProtectedRecruteurRoute; 