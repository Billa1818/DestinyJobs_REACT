...
const JobStats = ({ offres }) => {
  // Calculer les statistiques basées sur les données réellement disponibles dans l'API
  const stats = {
    total: offres.length,
    approved: offres.filter(o => o.status === 'APPROVED').length,
    pending: offres.filter(o => o.status === 'PENDING_APPROVAL').length,
    closed: offres.filter(o => o.status === 'CLOSED').length,
    expired: offres.filter(o => o.status === 'EXPIRED').length,
