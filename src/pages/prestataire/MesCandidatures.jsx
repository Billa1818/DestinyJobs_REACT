...
const MesCandidatures = () => {
  ...
  const fetchUserApplications = async () => {
    try {
      setLoading(true);
      const applicationsData = await dataService.getUserApplicationsByType(user.id, 'consultation');
  ...
  const filteredApplications = applications.filter(application => {
    ...
    const matchesSearch = !filters.search || 
      application.consultationTitle?.toLowerCase().includes(filters.search.toLowerCase()) ||
      application.domaine?.toLowerCase().includes(filters.search.toLowerCase());
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Suivez l'état de vos candidatures aux consultations</p>
          </div>
          <Link 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="Titre de la consultation ou domaine..."
            />
          </div>
          <div>
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              {applications.length === 0 
                ? "Vous n'avez pas encore postulé à des consultations."
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900">{application.consultationTitle || 'Consultation'}</h3>
                        <p className="text-sm text-gray-600">{application.domaine}</p>
                      </div>
                    ...
                    <div className="mt-3 text-sm text-gray-600">
                      ...
                      {application.proposedBudget && (
                        <p className="mb-2">
                          <strong>Budget proposé :</strong> {application.proposedBudget.toLocaleString()} FCFA
                        </p>
                      )}
                      {application.timeline && (
                        <p className="mb-2">
                          <strong>Délai proposé :</strong> {application.timeline}
                        </p>
                      )}
                      {application.methodology && (
                        <p className="mb-2">
                          <strong>Méthodologie :</strong> {application.methodology.substring(0, 100)}...
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2 ml-4">
                    ...
                    >
                      <i className="fas fa-eye mr-1"></i>Voir la consultation
                    </Link>
                  </div>
                </div>
              </div>
